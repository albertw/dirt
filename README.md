...
Dirt Authors:
Alf Salte (Alf) <salte@ifi.uio.no>, and G.Sorseth (Nicknack) <gjermund@ifi.uio.no>.
...
5. run bin/restart  (a program that runs in the background and
   ...
   The executable is bin/aberd (aberd = aber-daemon), it will be started by restart. (but you don't _have_ to use restart of course)


Examine the =README= file in the data directory to find out what the various files that are there are for, and edit the the bulletins, etc... 
...
- `-c` — Clear the syslog file on startup  
- `-f` — Run in the foreground (for debugging)  
- `-d <path>` — Location of the data dir (default: `../data`)  
- `-p <port>` — Port number (default: `6715`)  
- `-k` — Kill any other mud if it's already running  
- `-v` — Print version and exit  
- `-n <number>` — Maximum number of players (default: `32`)  
...
This version of this DIRT 3.2.1 code was provided by Alf. Changes in this git repo are to enable it to vompile and run with modern compilers and hardware including a preprocessor to replace the now oboslete 'cpp -R' preprocessing of the zone files. ~albertw